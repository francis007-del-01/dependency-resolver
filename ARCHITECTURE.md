...
A one-shot CLI that updates a single target repo's `pom.xml` based on runtime selectors. Jenkins invokes the resolver per `(owner, repo, branch)` plus release `groupId` and/or exact artifact selection. Resolver writes changes through pull requests (new branch per run), not direct base-branch commits.

## Flow

```
Jenkins (OWNER, REPO, BRANCH, RELEASE_GROUP_IDS/RELEASE_ARTIFACTS)
        │
        ▼
ResolverRunner (ApplicationRunner)
        ...
        │
        ├─ 2) PomManager.listCoordinatesForTargets(...)
...
| `pom/PomManager.java` | Maven model parsing, coordinate discovery by selector (`groupId` and/or exact `groupId:artifactId`), bump detection and in-place application. |
...
- Dependency scope is selected by runtime release selectors.
- Missing selector matches are warnings, not failures.
